...
def config_dict():
    """Build a dict of all config values for Jinja2 templates."""
    d = {}
    for key in ('camera_name', 'rotation', 'time_before_first_image', 'time_before_image',
                 'output_width', 'output_height', 'output_extension',
                 'embed_timestamp', 'embed_camera_name', 'file_name',
                 'text_size', 'text_color', 'text_background',
                 'ftp-mode', 'ftp-server', 'ftp-port', 'ftp-username', 'ftp-password',
                 'ftp-destination', 'camera_timezone', 'camera_daylight_savings',
                 'camera_port', 'camera_url', 'archive_retention_days', 'reserved_space_gb'):
        d[key] = globals().get(key)
    d['active_tab'] = globals().get('_active_tab', 'camera')
    return d


@app.route('/config.html')
def config():
    return render_template('config.html', config=config_dict(), rotation=ROTATION, active_tab=globals().get('_active_tab', 'camera'))
